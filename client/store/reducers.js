/*
  Reducers - decides how the change a state after receiving an action,
  and thus can be considered the entrance of a state change. A reducer is
  comprised of functions and it changes states by taking an action as argument,
   in which it then returns a new state.
  The actions get sent to App component and other parent component where
  they can be pass through as props.
*/
...
/*
  ...
  inside ProjectDetails component, we dispatch 'addUsers' to display users
  at initial load inside UserDetails component, we dispatch 'dispatchPairing'
  when user select a partner to pair with
*/
...
/*
  ...
  inside ProjectDetails component we dispatch 'CHANGE_PROJECT_INTEREST'
  when user selects 'they are interested'
  inside UserDetails component we dispatch 'CHANGE_USER' when user select
  'they want to pair' button
*/
...
/*
  first condition is the initial state
  inside UserDetails component we dispatch 'MESSAGE_SEND' when user sends a
  message to pairing user inside UserDetails component we dispatch
  'MESSAGE_LOAD' when user refreshes page to view new incoming messages
...
/*
  first condition is the initial state
  inside Project component, we dispatch 'PROGRESS_LOAD_ITEMS'
  to load the user's latest progress on project
